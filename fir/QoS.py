...
def output_to_wav(filename, testbenchNo):
		f=open(filename)
		f1=open('%s/simulation/testcase%d/converted.txt'%(DESIGN_DIR,testbenchNo),'w')
...
def dataCompare(generation, selNo, vLogFile, numTestCases):
	...
	for j in range(1,numTestCases+1):
		filename='%s/simulation/testcase%d/output.txt'%(DESIGN_DIR, j)
		if (os.path.exists(filename)):
	...
	for k in range(1,numTestCases+1):
		filename='%s/simulation/testcase%d/Ground_truth_train%d.txt'%(DESIGN_DIR,k,k)
		if (os.path.exists(filename)):
			f1 = open(filename)
			temp = []
			temp = f1.readlines()
			...
			status='fail'
		filename='%s/simulation/testcase%d/converted.txt'%(DESIGN_DIR,k)
		if (os.path.exists(filename)):
			f2=open('%s/simulation/testcase%d/converted.txt'%(DESIGN_DIR,k))
			tempconverted = []
	...
	dump_file='%s/Population/FilesInfo_G%d.txt'%(DESIGN_DIR,generation)
...
def main(generation, selNo, vLogFile, numTestCases):
	...
	status=dataCompare(generation,selNo,vLogFile,numTestCases)
