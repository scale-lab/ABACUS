...
def extractNumber(List):
    ...
    return temp
def QoS(Original, Modified, base, Generation, SelNo, VlogFile, node):
    im_width = 352
    im_height = 288
    status = 'success'
...
def dataCompare(base, Generation, SelNo, VlogFile, Num_testCases, node):
    status = 'success'
    ...
    for i in range (1, Num_testCases + 1):
        ...
        Modified='%s/simulation_%d/testcase%d/psnr.txt' % (DESIGN_DIR, node, i)
    ...
    if (status == 'success'):
        try:
            QoS(Original_MSE, Modified_MSE,base, Generation, SelNo, VlogFile, node)
        except:
            status = 'fail'
    return status
#print dataCompare(0,0,1,1,4)
def main(base, Generation, SelNo, VlogFile, Num_testCases, node):

    start_time=time.clock()

    status=dataCompare(base, Generation, SelNo, VlogFile, Num_testCases, node)
