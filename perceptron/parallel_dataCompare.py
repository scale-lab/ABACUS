...
import math
import numpy
def QoS (Original_data, Modified_data, Ground_truth, Generation, SelNo, VlogFile, node):
    ...
    file_to_write = '%s/Population/node%d.txt' % (DESIGN_DIR, node)
...
def dataCompare(Generation, SelNo, VlogFile, NumtestCases, node):
	...
	for i in range(1, NumtestCases + 1):
		...
		Modified = '%s/simulation_%d/testcase%d/out_labels.txt' % (DESIGN_DIR,node, i)
		Groundtruth = '%s/simulation_%d/testcase%d/Ground_truth.txt' % (DESIGN_DIR,node, i)
		...
		if (os.path.exists(Modified)):
			...
			print "No output file for Modified design. Data analysis break..."
			status = 'fail'
	...
	if (status == 'success'):
		try:
			print "here"
			status = QoS (Original_data, Modified_data, Ground_truth, Generation, SelNo, VlogFile, node)
		except:
			status = 'fail'
	print "status:\n ", status
def main(Generation, SelNo, VlogFile, NumtestCases, node):
    dataCompare(Generation, SelNo, VlogFile, NumtestCases, node)
